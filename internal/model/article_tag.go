...
func (a ArticleTag) ListByIDs(db *gorm.DB, articleIDs []uint32) ([]*ArticleTag, error) {
	var articleTags []*ArticleTag
	err := db.Where("tag_id IN (?) AND is_del = ?", articleIDs, 0).Find(&articleTags).Error
...
func (a ArticleTag) DeleteOne(db *gorm.DB) error {
	if err := db.Where("id = ? AND is_del = ?", a.ArticleID, 0).Delete(&a).Limit(1).Error; err != nil {
		return err
	}
	return nil
}
